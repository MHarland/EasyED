...
for u, fname in zip(us, fnames):
    ...
    for beta in betas:
        ...
        n_tot = nsum([c[s, i].H.dot(c[s, i]) for s in ['up', 'dn'] for i in range(4)], axis = 0)
        ...
        for i, e, deg in zip(range(len(energies)), energies, degeneracies):
            ...
            ntots_i = [tetrahedron.hamiltonian.getSuperpositionStatesEnergySorted()[i][j].getQuantumNumber(n_tot) for j in range(n_states_e)]
            for s2, ntot in zip(s2s_i, ntots_i):
                if abs(ntot-4) > small: continue
                is_element = False
